...
export async function exportValidatorKeys({
  ...
  try {
    ...
    const validatorAccountsData = await shell(
      `docker exec ${validatorContainerName} validator accounts list --wallet-dir=${eth2migrationParams.keys.walletDir} \
--wallet-password-file=${eth2migrationParams.keys.walletPasswordFile} --${network} --accept-terms-of-use`
    ...
    await shell(
      `docker exec ${validatorContainerName} cp ${eth2migrationParams.keys.walletPasswordFile} ${eth2migrationParams.backup.backupDir}`
    ).catch(e => {
    ...
    await shell(
      `docker exec ${validatorContainerName} validator accounts backup --wallet-dir=${eth2migrationParams.keys.walletDir} \
...
--${network} --accept-terms-of-use`
    ...
    await shell(
      `docker exec ${validatorContainerName} sh -c 'apt update; apt install -y unzip; unzip -p ${eth2migrationParams.backup.backupKeysFile} -d ${eth2migrationParams.backup.backupDir}'`
    ).catch(e => {
